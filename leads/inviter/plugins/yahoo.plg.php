...
$_pluginInfo=array(
	...
	'imported_details'=>array('first_name','last_name','email_1','phone_1'),
	);
/**
 * Yahoo! Plugin
 * 
...
class yahoo extends openinviter_base
	...
	public function getMyContacts()
		...
		$res=str_replace(array('  ','	','						',PHP_EOL,"\n","\r\n","<br>","(mobile)","(work)","(home)","(",")"),array('','','','','','','','','','','',''),$res);
		preg_match_all("#\<tr class\=\"phead\"\>\<td colspan\=\"2\"\>(.+)\<\/tr\>(.+)\<div class\=\"first\"\>\<\/div\>\<div\>\<\/div\>(.+)\<\/div\>(.+)\<div\>(.+)\<\/div\>#U",$res,$bulk);
		if (!empty($bulk))
			{
			foreach($bulk[1] as $key=>$bulkName)
				...
				if (preg_match('/\&nbsp\;\-\&nbsp\;/',$nameFormated)) 
					...
					$name = preg_split('/(?=[A-Z])/', $emailA[0]);
					$first_name = trim($name[1]);
					$last_name = trim($name[2]);

					if (!empty($emailA[1])) $contacts[$emailA[1].'@yahoo.com']=array('first_name'=>$first_name,'last_name'=>$last_name,'email_1'=>$emailA[1].'@yahoo.com','phone_1'=>trim($emailA[2]));
					}
				elseif (!empty($bulk[3][$key])) { 
					$email=strip_tags(trim($bulk[3][$key])); 
					$phone=strip_tags(trim($bulk[4][$key]));
					$name = preg_split('/(?=[A-Z])/', $nameFormated);
					$first_name = trim($name[1]);
					$last_name = trim($name[2]);
					if(!empty($name[3])) $last_name .= ' '.$name[3]; 
					$contacts[$email]=array('first_name'=>$first_name,'last_name'=>$last_name,'email_1'=>$email,'phone_1'=>$phone); 
				}
				}
			}			
